/**
 * NOSTALGIA v26 — BEST OF v24 + v25
 *
 * Combines:
 * - v24's elliptical canopy (wide, flat, continuous) but with large-scale edge noise
 * - v25's warmer wall tones and atmospheric haze
 * - Scene blur radius 6 (v24 was 4-5, v25 was 8)
 * - Simple iron picket fence
 * - Dissolution concept preserved
 *
 * Key canopy change: add large-scale (150-200px) noise to break the perfect ellipse.
 * Real live oaks have irregular edges — bumps, gaps, asymmetric lobes.
 */
...
function buildScene(w:number, h:number, seed:number, rand:()=>number) {
      ...
      if (x >= 0 && x < w && y >= 0 && y < h) {
        ...
        if (isPortico) {
          ...
          if (dissolveVal < threshold) {
            ...
            if (edgeDist < 1) opacity[y*w+x] = Math.max(opacity[y*w+x], 0.06 * (1 - edgeDist))
          } else {
          ...
          if (dissolveVal < threshold) {
            ...
            if (edgeDist < 1) opacity[y*w+x] = Math.max(opacity[y*w+x], 0.10 * (1 - edgeDist))
          } else {
  ...
  for (let ci = 1; ci <= numColumns; ci++) {
    ...
    for (let y = Math.floor(colTopY - capH); y < Math.floor(colTopY + 3); y++) {
      ...
      for (let dx = -Math.ceil(capW * widthMult); dx <= Math.ceil(capW * widthMult); dx++) {
        ...
        if (px >= 0 && px < w && y >= 0 && y < h) {
          ...
          if (d < 1.0) { opacity[y*w+px] = Math.max(opacity[y*w+px], 0.88); brightness[y*w+px] = Math.max(brightness[y*w+px], 0.48) }
        }
      }
    }
    ...
    for (let y = Math.floor(columnBaseY - baseH); y < Math.floor(columnBaseY); y++) {
      for (let dx = -Math.ceil(baseW); dx <= Math.ceil(baseW); dx++) {
        const px = Math.round(colCX + dx)
        if (px >= 0 && px < w && y >= 0 && y < h) { opacity[y*w+px] = Math.max(opacity[y*w+px], 0.82); brightness[y*w+px] = Math.max(brightness[y*w+px], 0.44) }
      }
    }
  }
  ...
  for (let ci = 0; ci <= numColumns; ci++) {
    ...
    for (let y = Math.floor(colTopY + h*0.02); y < Math.floor(columnBaseY - h*0.01); y++) {
      for (let x = Math.floor(gapL); x <= Math.ceil(gapR); x++) {
        if (x >= 0 && x < w && y >= 0 && y < h) brightness[y*w+x] = Math.min(brightness[y*w+x], 0.03)
      }
    }
  }
  ...
  for (let bi = 0; bi < numColumns - 1; bi++) {
    ...
    const winW2 = colSpacing * 0.09; const winH2 = (columnBaseY - colTopY) * 0.28
  ...
  for (let si = 0; si < 4; si++) {
    const stepT = stepsTop + (stepsH / 4) * si; const stepB = stepsTop + (stepsH / 4) * (si + 1); const indent = si * porticoW * 0.010
    for (let y = Math.floor(stepT); y < Math.floor(stepB); y++) { for (let x = Math.floor(porticoL - indent); x <= Math.ceil(porticoR + indent); x++) { if (x >= 0 && x < w && y >= 0 && y < h) { opacity[y*w+x] = Math.max(opacity[y*w+x], 0.45); brightness[y*w+x] = Math.max(brightness[y*w+x], si % 2 === 0 ? 0.40 : 0.30) } } }
  }

  // ===== LIVE OAK — ELLIPTICAL canopy with LARGE-SCALE edge noise =====
  ...
  for (let y = Math.floor(treeBaseY); y > Math.floor(trunkTopY); y--) {
    ...
    for (let dx = -Math.ceil(tw); dx <= Math.ceil(tw); dx++) {
      ...
      if (px >= 0 && px < w && y >= 0 && y < h) { const d = Math.abs(dx) / tw; if (d < 1) { opacity[y*w+px] = Math.max(opacity[y*w+px], 0.95*(1-d*0.1)); brightness[y*w+px] = Math.min(brightness[y*w+px], 0.04) } }
    }
  }
  ...
  // Canopy — wide flat ellipse with LARGE-SCALE edge noise
  const canopyCX = treeCX + w * 0.01
  const canopyCY = h * 0.30
  const canopyRX = w * 0.34
  const canopyRY = h * 0.13
  // Large-scale noise to break the perfect ellipse
  const canopyShapeN = makeNoise(seed + 555, 180)    // BIG features
  const canopyShapeN2 = makeNoise(seed + 556, 90)    // medium features
  const canopyEdgeN = makeNoise(seed + 600, 30)       // small edge detail
  ...
  for (let y = Math.max(0, Math.floor(canopyCY - canopyRY * 1.8)); y <= Math.min(h-1, Math.ceil(canopyCY + canopyRY * 2.0)); y++) {
    for (let x = Math.max(0, Math.floor(canopyCX - canopyRX * 1.3)); x <= Math.min(w-1, Math.ceil(canopyCX + canopyRX * 1.3)); x++) {
      ...
      let d = dx2*dx2 + dy2*dy2

      // LARGE-SCALE shape distortion — makes the ellipse irregular
      d += canopyShapeN(x, y) * 0.35    // big bumps/indentations
      d += canopyShapeN2(x, y) * 0.18   // medium variation
      d += canopyEdgeN(x, y) * 0.12     // fine detail

      // Bottom droop
      if (dy2 > 0.15) {
        const droop = (dy2 - 0.15) * 0.75
  ...
  for (let i = 0; i < 140; i++) {
    const mx0 = treeCX + (rand() - 0.5) * w * 0.58; const my0 = h * 0.18 + rand() * h * 0.28
    const mLen = 70 + rand() * 160; const mW = 2.5 + rand() * 3.5
    for (let my = 0; my < mLen; my++) {
      const t = my / mLen; const sway = mossN(mx0 + i * 77, my0 + my) * 22 * t
      const mx = mx0 + sway, myy = my0 + my; if (myy >= h) continue
      const op = (1 - t*t) * 0.52; const mw = mW * (1 - t * 0.45)
      for (let ddx = -Math.ceil(mw); ddx <= Math.ceil(mw); ddx++) {
        const px = Math.round(mx + ddx); if (px < 0 || px >= w) continue; const py = Math.round(myy)
        if (py >= 0 && py < h) { const d2 = Math.abs(ddx) / mw; if (d2 < 1) { const v = (1-d2) * op; opacity[py*w+px] = Math.max(opacity[py*w+px], v); brightness[py*w+px] = Math.min(brightness[py*w+px], 0.04) } }
      }
    }
  }
...
async function main() {
  ...
  const seeds: Record<string, number> = { a: 26001, b: 26002, c: 26003, d: 26004 }
  const seed = seeds[variant] ?? 26001
  const rand = makePRNG(seed)

  console.log(`=== NOSTALGIA v26 variant ${variant} (seed: ${seed}) ===`)
  ...
  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W; x++) {
      ...
      if (op > 0.01) {
        ...
        const scR = dR + (aR-dR)*br; const scG = dG + (aG-dG)*br; const scB = dB + (aB-dB)*br
        const a = Math.min(1, op); r = r*(1-a) + scR*a; g = g*(1-a) + scG*a; b = b*(1-a) + scB*a
      }
      const gl = glB[i]
  ...
  const fn = `output/nostalgia-v26-${variant}.png`
