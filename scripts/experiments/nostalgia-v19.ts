/**
 * NOSTALGIA v19 — DUSK SILHOUETTE
 *
 * Building on v18's classical architecture. Key changes:
 * 1. Wall MUCH darker — building reads as dark mass with columns catching last light
 * 2. Columns thicker + brighter — they're the light-catching elements
 * 3. Building taller (corniceY 0.30 → 0.26) — more imposing presence
 * 4. Selective window lighting — not all windows lit (some dark, some warm)
 * 5. Stronger window glow — the warmth spills further
 * 6. Door with warm light spilling out at ground level
 *
 * The feeling: dusk, building mostly in shadow, warm light from inside.
 * You're across the street watching the building settle into evening.
 */
...
function buildScene(w:number, h:number, seed:number, rand:()=>number) {
  ...
  // Main wall behind columns — DARK at dusk, mostly in shadow
  for (let y = Math.floor(corniceY - 5); y < Math.floor(columnBaseY); y++) {
    for (let x = Math.floor(bldgL); x <= Math.ceil(bldgR); x++) {
      if (x >= 0 && x < w && y >= 0 && y < h) {
        ...
        brightness[y*w+x] = Math.max(brightness[y*w+x], 0.22)  // dark — building in shadow
      }
    }
  }
  ...
  for (let y = Math.floor(pedimentPeakY); y < Math.floor(pedimentBaseY); y++) {
    ...
    for (let x = Math.floor(bldgCX - hw); x <= Math.ceil(bldgCX + hw); x++) {
      if (x >= 0 && x < w && y >= 0 && y < h) {
        ...
        brightness[y*w+x] = Math.max(brightness[y*w+x], 0.32)  // catching some light
      }
    }
  }
    ...
    for (let y = Math.floor(colTopY); y < Math.floor(columnBaseY); y++) {
      ...
      for (let dx = -Math.ceil(hw + 2); dx <= Math.ceil(hw + 2); dx++) {
        ...
        if (px >= 0 && px < w && y >= 0 && y < h) {
          ...
          if (d < 0.85) {
            // Column catches last golden light — bright against dark wall
...
async function main() {
  ...
  const seeds: Record<string, number> = { a: 19001, b: 19002, c: 19003, d: 19004 }
  const seed = seeds[variant] ?? 19001
  ...
  const fn = `output/nostalgia-v19-${variant}.png`
