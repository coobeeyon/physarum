/**
 * NOSTALGIA v21 — GHOST BUILDING (informed by actual Carrollton Courthouse)
 *
 * The real building: 4 colossal fluted Ionic columns in a forward portico,
 * triangular pediment, two-story Greek Revival. Deep shadow behind the columns.
 * Weathered stucco, grey-green patina. Trees framing both sides.
 *
 * Changes from v20:
 * - 4 columns instead of 6, thicker and more imposing
 * - Building taller and narrower (matching real proportions)
 * - Deep portico shadow zone behind columns
 * - Variable wall dissolution — some areas almost gone, others partially solid
 * - Columns fluted (alternating light/dark vertical strips)
 * - Stronger golden-hour light on left-facing column surfaces
 * - Portico creates the sense of depth the real building has
 */
...
function buildScene(w:number, h:number, seed:number, rand:()=>number) {
  ...
  // Dissolution noise — varies how "gone" different parts of the wall are
  const dissolveN = makeFBM(seed + 1100, 120, 4)
  ...
  // ===== REAR WALL (behind the portico) — variable dissolution =====
  for (let y = Math.floor(corniceY - 5); y < Math.floor(columnBaseY); y++) {
    for (let x = Math.floor(bldgL); x <= Math.ceil(bldgR); x++) {
      if (x >= 0 && x < w && y >= 0 && y < h) {
        // Dissolution varies across the facade
        const dn = dissolveN(x, y) * 0.5 + 0.5  // 0 to 1
        const isPortico = x >= porticoL && x <= porticoR

        if (isPortico) {
          // Wall behind portico: heavily dissolved, deep shadow
          // The portico creates depth — wall is recessed
          const wallOp = dn < 0.35 ? 0.08 :  // mostly gone
                         dn < 0.55 ? 0.25 :  // faint ghost
                         dn < 0.75 ? 0.45 :  // partially present
                                     0.60    // mostly solid
          opacity[y*w+x] = Math.max(opacity[y*w+x], wallOp)
          brightness[y*w+x] = Math.max(brightness[y*w+x], 0.10)  // deep shadow
        } else {
          // Side walls: less dissolved, catching more light
          const wallOp = dn < 0.25 ? 0.20 :
                         dn < 0.50 ? 0.38 :
                         dn < 0.75 ? 0.52 :
                                     0.65
          const sideBr = x < porticoL ? 0.35 : 0.22  // left side catches light
          opacity[y*w+x] = Math.max(opacity[y*w+x], wallOp)
  ...
  const colWidth = colSpacing * 0.22  // thicker columns
  ...
  for (let ci = 0; ci <= numColumns; ci++) {
    ...
    for (let y = Math.floor(colTopY + h*0.02); y < Math.floor(columnBaseY - h*0.01); y++) {
      for (let x = Math.floor(gapL); x <= Math.ceil(gapR); x++) {
        if (x >= 0 && x < w && y >= 0 && y < h) {
          // Deep shadow behind columns — the portico void
          brightness[y*w+x] = Math.min(brightness[y*w+x], 0.08)
        }
      }
    }
  }
...
async function main() {
  ...
  const seeds: Record<string, number> = { a: 21001, b: 21002, c: 21003, d: 21004 }
  const seed = seeds[variant] ?? 21001
  const rand = makePRNG(seed)

  console.log(`=== NOSTALGIA v21 variant ${variant} (seed: ${seed}) ===`)
  ...
  const fn = `output/nostalgia-v21-${variant}.png`
