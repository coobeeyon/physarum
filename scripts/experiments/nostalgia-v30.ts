/**
 * NOSTALGIA v28 — MOSS ON SEPARATE LAYER
 *
 * Based on v27. Key fix: Spanish moss is drawn on its own buffer and blurred
 * at radius 2 (not 6 like the scene). This means the moss strands stay visible
 * and defined — hanging curtains you can actually see — while the building
 * and canopy behind them stay dreamy/blurred.
 *
 * Also: moss strands are wider (8-18px) and more opaque (0.80).
 * More strands, longer drapes. This is what makes it New Orleans.
 */
...
function buildScene(w:number, h:number, seed:number, rand:()=>number) {
  ...
  for (let x = 0; x < w; x++) {
    ...
    for (let y = Math.floor(groundY + gv); y < h; y++) {
      if (y >= 0) {
        ...
        // Sunlit ground: the golden hour reaches the grass and the viewer's
        // side of the fence, warmer toward the bottom of frame
        const sunReach = 0.36 + ((y - groundY) / (h - groundY)) * 0.18
        brightness[y*w+x] = sunReach + gTex * 0.08
      }
    }
  }
  ...
  for (let x = 0; x < w; x++) {
    for (let y = Math.floor(groundY - h*0.020); y < Math.floor(groundY); y++) {
      if (y >= 0 && y < h) {
        ...
        brightness[y*w+x] = Math.max(brightness[y*w+x], 0.58)
      }
    }
  }
  ...
  // Limbs — return info for moss placement
  ...
  return { opacity, brightness, glow, limbInfos, canopyMask, canopyCX, canopyCY, canopyRX, canopyRY }
}
...
function buildMoss(w:number, h:number, seed:number, rand:()=>number,
  ...
  canopyMask: Uint8Array, canopyCX:number, canopyCY:number, canopyRX:number, canopyRY:number) {

  const mossOp = new Float32Array(w * h)  // moss opacity (separate layer)
  const mossN = makeNoise(seed + 800, 8)
  const mossN2 = makeNoise(seed + 810, 18)
  const mossN3 = makeNoise(seed + 820, 40)  // large-scale clumping
  const groundY = h * 0.82

  // Canopy top: moss only from points BELOW this line
  const canopyTopY = canopyCY - canopyRY * 0.6
  // Moss only within tree's horizontal extent (not over building)
  const treeLeftLimit = canopyCX - canopyRX * 1.1
  const treeRightLimit = canopyCX + canopyRX * 0.6  // don't extend too far toward building
  ...
  for (const limb of limbInfos) {
    for (let pi = 12; pi < limb.pts.length; pi += 4 + Math.floor(rand() * 7)) {
      ...
      // SKIP moss above canopy top — prevents dark spikes sticking up
      if (ay < canopyTopY) continue
  ...
  for (let i = 0; i < 60; i++) {
    const mx0 = canopyCX + (rand() - 0.55) * canopyRX * 1.6  // bias left (away from building)
...
function buildIron(w: number, h: number, _seed: number) {
  ...
  for (let i = 0; i < numPickets; i++) {
    ...
    // Rounded ball finial — domestic New Orleans iron, not institutional spikes
    // (the v28 blind read saw "exclusion"; spear points were part of why)
    const ballR = barThick * 0.85; const ballCY = railTop - ballR - 2
    for (let y = Math.floor(ballCY - ballR - 1); y <= Math.ceil(ballCY + ballR + 1); y++) { if (y < 0 || y >= h) continue; for (let x = Math.floor(px - ballR - 1); x <= Math.ceil(px + ballR + 1); x++) { if (x < 0 || x >= w) continue; const d = Math.sqrt((x-px)**2 + (y-ballCY)**2); if (d < ballR) iron[y*w+x] = Math.max(iron[y*w+x], 1.0); else if (d < ballR + 1.2) iron[y*w+x] = Math.max(iron[y*w+x], 1.0 - (d - ballR) / 1.2) } }
    fillBar(iron, w, h, px, ballCY + ballR, px, railTop, barThick * 0.6)
  }
  // No solid fill below the fence: the viewer stands on a lit sidewalk, not
  // in a black void (the void was the other half of "ominous").
  return iron
}

async function main() {
  const variant = process.argv[2] || "a"
  const seeds: Record<string, number> = { a: 28001, b: 28002, c: 28003, d: 28004 }
  const seed = seeds[variant] ?? 28001
  const rand = makePRNG(seed)

  console.log(`=== NOSTALGIA v30 variant ${variant} (seed: ${seed}) ===`)
  ...
  const { opacity, brightness, glow: glowMap, limbInfos, canopyMask, canopyCX, canopyCY, canopyRX, canopyRY } = buildScene(W, H, seed, rand)

  console.log("  Blurring scene (radius 5)...")
  const opB = gaussianBlur(opacity, W, H, 5)
  const brB = gaussianBlur(brightness, W, H, 6)
  const glB = gaussianBlur(glowMap, W, H, 28)

  // Fireflies: sharp warm points of light drifting between fence and scene.
  // The only pure-bright sharp detail in the image — focal points that say
  // summer evening (the v28 read found no focal point and no warmth).
  // Fireflies register only against dark mass — put them in front of the
  // tree canopy and moss curtains, a few low near the fence line
  const fireflies: Array<{ x: number; y: number; s: number }> = []
  const nFlies = 7 + Math.floor(rand() * 3)
  for (let i = 0; i < nFlies; i++) {
    const inTree = rand() < 0.7
    fireflies.push(
      inTree
        ? { x: W * (0.02 + rand() * 0.4), y: H * (0.22 + rand() * 0.38), s: 0.75 + rand() * 0.25 }
        : { x: W * (0.1 + rand() * 0.75), y: H * (0.6 + rand() * 0.12), s: 0.6 + rand() * 0.3 },
    )
  }

  console.log("  Building moss layer...")
  const mossRaw = buildMoss(W, H, seed, rand, limbInfos, canopyMask, canopyCX, canopyCY, canopyRX, canopyRY)
  ...
  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W; x++) {
      ...
      // Layer 3.5: fireflies — sharp bright cores with soft warm halos
      for (const f of fireflies) {
        const dx = x - f.x, dy = y - f.y
        const d2 = dx*dx + dy*dy
        if (d2 < 3600) {
          const d = Math.sqrt(d2)
          const core = Math.max(0, 1 - d / (5.5 * f.s))
          const halo = Math.max(0, 1 - d / (52 * f.s)) ** 2 * 0.65
          const a = Math.min(1, core + halo) * f.s
          r = r + (255 - r) * a
          g = g + (236 - g) * a
          b = b + (140 - b) * a
        }
      ...
      // Layer 4: iron fence (sharp, no blur) — warm-dark with golden glints
      const ironVal = ironSil[i]
      if (ironVal > 0.01) { const a = Math.min(1, ironVal); const it = texN(x*3, y*3)*0.5+0.5; const glint = Math.max(0, it - 0.62) * 2.2; r = r*(1-a) + (26+it*8+glint*70)*a; g = g*(1-a) + (18+it*6+glint*48)*a; b = b*(1-a) + (14+it*4+glint*20)*a }

      // Film grain + gentle vignette
      const grain = grainN(x,y)*3.5; r += grain; g += grain*0.8; b += grain*0.6
      const cx2 = x/W-0.5, cy2 = y/H-0.36; const vig = 1.0 - (cx2*cx2*0.55 + cy2*cy2*0.95)*0.16
      r *= vig; g *= vig; b *= vig
  ...
  const fn = `output/nostalgia-v30-${variant}.png`
