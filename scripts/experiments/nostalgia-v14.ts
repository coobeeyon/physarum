/**
 * NOSTALGIA v14 — THROUGH THE IRON (refined)
 *
 * Fixes from v13:
 * - Iron railing is LARGER (bottom 40% of frame, not 30%)
 * - Tree canopy connects to trunk, has more sky holes
 * - Scene blur reduced (atmospheric, not foggy)
 * - Thicker iron curves with more weight
 * - Scene elements better positioned
 */
...
function buildScene(w: number, h: number, seed: number, rand: () => number): Float32Array {
  ...
  // Trunk — extends up into the canopy region
  const trunkTopY = h * 0.30
  for (let y = Math.floor(groundY); y > Math.floor(trunkTopY); y--) {
    const t = (groundY - y) / (groundY - trunkTopY)
    const tw = 14 * (1.5 - t * 0.7)
    const lean = t * 20
    for (let x = Math.floor(treeCX + lean - tw); x <= Math.ceil(treeCX + lean + tw); x++) {
      if (x >= 0 && x < w && y >= 0 && y < h) scene[y * w + x] = Math.max(scene[y * w + x], 0.92)
    }
  }

  // Major branches extending from trunk into canopy
  for (const branch of [
    { angle: -0.8, len: 0.22 },
    { angle: -1.8, len: 0.18 },
    { angle: -0.3, len: 0.25 },
    ...
    const bStartY = trunkTopY + h * 0.06
    const bStartX = treeCX + 15
    for (let s = 0; s < 60; s++) {
      const bt = s / 60
      const bx = bStartX + Math.cos(branch.angle) * w * branch.len * bt
      const by = bStartY + Math.sin(branch.angle) * h * branch.len * bt
      const bw = 10 * (1 - bt * 0.6)
      for (let dx = -Math.ceil(bw); dx <= Math.ceil(bw); dx++) {
        for (let dy = -Math.ceil(bw); dy <= Math.ceil(bw); dy++) {
          const px = Math.round(bx + dx), py = Math.round(by + dy)
          if (px >= 0 && px < w && py >= 0 && py < h && Math.sqrt(dx*dx+dy*dy) < bw) {
            scene[py * w + px] = Math.max(scene[py * w + px], 0.88)
          }
        }
      }
    }
  ...
  // Canopy — more blobs, connected to branches, with sky gaps
  const blobCenters = [
    { x: treeCX - treeSpread * 0.30, y: h * 0.18, rx: treeSpread * 0.18, ry: h * 0.10 },
    { x: treeCX - treeSpread * 0.10, y: h * 0.13, rx: treeSpread * 0.22, ry: h * 0.12 },
    { x: treeCX + treeSpread * 0.12, y: h * 0.15, rx: treeSpread * 0.24, ry: h * 0.13 },
    { x: treeCX + treeSpread * 0.32, y: h * 0.22, rx: treeSpread * 0.17, ry: h * 0.10 },
    { x: treeCX + treeSpread * 0.05, y: h * 0.22, rx: treeSpread * 0.16, ry: h * 0.09 },
    { x: treeCX - treeSpread * 0.18, y: h * 0.26, rx: treeSpread * 0.14, ry: h * 0.08 },
    // Connecting blobs near trunk
    { x: treeCX + 10, y: h * 0.28, rx: treeSpread * 0.12, ry: h * 0.07 },
    { x: treeCX - 5, y: h * 0.32, rx: treeSpread * 0.10, ry: h * 0.06 },
  ]
...
async function main() {
  ...
  const seeds: Record<string, number> = { a: 14001, b: 14002, c: 14003, d: 14004 }
  ...
  const filename = `output/nostalgia-v14-${variant}.png`
