/**
 * NOSTALGIA v20 — GHOST BUILDING
 *
 * The building that was torn down. You can see the sky through it.
 * Opacity reduced so the golden hour sky bleeds through the structure.
 * Columns are the most opaque elements — they catch the last light.
 * Glow blurred much more (radius 35) — warm light spreads like memory.
 * Shape blurred less (radius 7) — the architecture is still recognizable.
 *
 * The feeling: the building is already half-gone. A place shaped people,
 * then got erased. The columns are the last things to dissolve.
 */
...
function buildScene(w:number, h:number, seed:number, rand:()=>number) {
  ...
  const pedimentPeakY = corniceY - bldgW * 0.09  // slightly shallower — Greek proportions
  ...
  for (let y = Math.floor(pedimentPeakY); y < Math.floor(pedimentBaseY); y++) {
    ...
    for (let x = Math.floor(bldgCX - hw); x <= Math.ceil(bldgCX + hw); x++) {
      if (x >= 0 && x < w && y >= 0 && y < h) {
        opacity[y*w+x] = Math.max(opacity[y*w+x], 0.50)  // pediment translucent too
        brightness[y*w+x] = Math.max(brightness[y*w+x], 0.32)
      }
    }
  }
  ...
  for (let y = Math.floor(corniceY - entH); y < Math.floor(corniceY + entH); y++) {
    for (let x = Math.floor(bldgL - 15); x <= Math.ceil(bldgR + 15); x++) {
      if (x >= 0 && x < w && y >= 0 && y < h) {
        opacity[y*w+x] = Math.max(opacity[y*w+x], 0.65)  // entablature still reads
  ...
  const colWidth = colSpacing * 0.20  // thicker — reads clearly through blur
    ...
    for (let y = Math.floor(colTopY); y < Math.floor(columnBaseY); y++) {
      ...
      for (let dx = -Math.ceil(hw + 2); dx <= Math.ceil(hw + 2); dx++) {
        ...
        if (px >= 0 && px < w && y >= 0 && y < h) {
          ...
          if (d < 0.85) {
            // Columns are the most opaque — last to dissolve
            opacity[y*w+px] = Math.max(opacity[y*w+px], 0.82)
            // Bright on the light-facing side, darker on the shadow side
            const sideLit = dx < 0 ? 0.80 : 0.52  // left side catches more light
            ...
            brightness[y*w+px] = Math.min(brightness[y*w+px], 0.15)
          }
        }
      }
    }
  ...
  // Windows — selective warm glow between columns
  // Not all windows lit — some dark (more natural, more melancholic)
  const litWindows = [true, false, true, true, false]  // which bays are lit
  const winW2 = colSpacing * 0.14  // slightly wider windows
  const winH2 = (columnBaseY - colTopY) * 0.30  // taller windows
  for (let wi = 0; wi < numColumns - 1; wi++) {
    if (!litWindows[wi]) continue  // skip dark windows
    const wx = bldgL + (wi + 1.5) * colSpacing
    const wy = colTopY + (columnBaseY - colTopY) * 0.18
    for (let y = Math.floor(wy); y < Math.floor(wy + winH2); y++) {
      if (y < 0 || y >= h) continue
      for (let x = Math.floor(wx - winW2); x <= Math.ceil(wx + winW2); x++) {
        if (x < 0 || x >= w) continue
        glow[y*w+x] = 1.0  // maximum warmth
      }
    }
    // Window halo — bigger, warmer, light spilling out into the evening
    const haloR = winW2 * 7
    for (let dy = -haloR; dy <= haloR; dy++) {
      for (let dx = -haloR; dx <= haloR; dx++) {
        ...
        if (px >= 0 && px < w && py >= 0 && py < h) {
          ...
          if (d < 1) glow[py*w+px] = Math.max(glow[py*w+px], (1-d*d) * 0.70)
        }
      }
    }
  }

  // Door — warm light from main entrance
  const doorCX = bldgCX
  const doorTop = columnBaseY - (columnBaseY - colTopY) * 0.55
  const doorW = colSpacing * 0.18
  const doorH = (columnBaseY - doorTop)
  for (let y = Math.floor(doorTop); y < Math.floor(columnBaseY); y++) {
    for (let x = Math.floor(doorCX - doorW); x <= Math.ceil(doorCX + doorW); x++) {
      if (x >= 0 && x < w && y >= 0 && y < h) {
        glow[y*w+x] = Math.max(glow[y*w+x], 0.75)
      }
    }
  }
  // Door light halo
  const doorHaloR = doorW * 6
  for (let dy = -doorHaloR; dy <= doorHaloR; dy++) {
    for (let dx = -doorHaloR; dx <= doorHaloR; dx++) {
      const px = Math.round(doorCX + dx), py = Math.round(doorTop + doorH*0.5 + dy)
      if (px >= 0 && px < w && py >= 0 && py < h) {
...
async function main() {
  ...
  const seeds: Record<string, number> = { a: 20001, b: 20002, c: 20003, d: 20004 }
  const seed = seeds[variant] ?? 20001
  ...
  const opB = gaussianBlur(opacity, W, H, 8)   // tighter shape — architecture recognizable
  const brB = gaussianBlur(brightness, W, H, 10)
  const glB = gaussianBlur(glowMap, W, H, 35)  // wide glow — warm light spreads like memory
  ...
  const fn = `output/nostalgia-v20-${variant}.png`
