/**
 * CONVERSATION PIECE v4 — thumbnail life in the dusk register
 *
 * v3 verdicts: the time-split reads at 280px; hills crossing the window seam
 * are the stitch that makes it one scene; the window must clearly overlap the
 * ground so block flowers live inside the memory. Weakness: the dusk
 * foreground reads as featureless dark mud at thumbnail.
 *
 * v4: warm the foreground toward umber, scale dusk flowers up with nearness,
 * raise their presence, make closed buds legible, dim stars slightly.
 */
...
function renderDusk(scene: Scene, seed: number, stars: boolean): Float32Array {
	...
	for (const f of scene.flowers) {
		...
		const col = mix([trueC[0] * 0.88, trueC[1] * 0.78, trueC[2] * 0.78], [255, 195, 135], 0.14)
...
function placeWindow(scene: Scene, aw: number, ah: number): { ax0: number; ay0: number; aw: number; ah: number } {
	...
	let ay0 = groundRow + 12 - ah // 12 rows of ground inside
...
for (const seed of [707, 808, 505, 1111, 1313, 1717, 1919, 2323]) {
	const scene = drawScene(seed)
	const atari = renderAtari(scene)
	const dusk = renderDusk(scene, seed, true)
	composite(scene, atari, dusk, placeWindow(scene, 75, 45), `output/conversation-piece-v4-s${seed}.png`)
}
