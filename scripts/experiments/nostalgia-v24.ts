/**
 * NOSTALGIA v24 — REFINED COMPOSITION
 *
 * Building on v23's correct iron fence and larger building:
 * 1. Live oak canopy MUCH wider and flatter — horizontal spread, not round
 * 2. Less scene blur — building recognizable, not just a silhouette
 * 3. Dissolution slightly more varied — some larger patches, some smaller
 * 4. Warmer overall tone — deeper golden hour
 * 5. Moss more visible as vertical draping
 */
...
function buildScene(w:number, h:number, seed:number, rand:()=>number) {
  ...
  const dissolveN = makeFBM(seed + 1100, 180, 5)   // larger scale = bigger patches
  const dissolveN2 = makeFBM(seed + 1200, 60, 3)   // smaller detail layer
  ...
  for (let x = 0; x < w; x++) {
    ...
    for (let y = Math.floor(groundY + gv); y < h; y++) {
      if (y >= 0) { opacity[y*w+x] = 0.88; brightness[y*w+x] = 0.14 }
    }
  }

  // Sidewalk
  for (let x = 0; x < w; x++) {
    for (let y = Math.floor(groundY - h*0.022); y < Math.floor(groundY); y++) {
      if (y >= 0 && y < h) {
        opacity[y*w+x] = Math.max(opacity[y*w+x], 0.38)
        brightness[y*w+x] = Math.max(brightness[y*w+x], 0.46)
      }
    }
  }
      ...
      if (x >= 0 && x < w && y >= 0 && y < h) {
        ...
        if (isPortico) {
          ...
          if (dissolveVal < threshold) {
            ...
            if (edgeDist < 1) {
              opacity[y*w+x] = Math.max(opacity[y*w+x], 0.08 * (1 - edgeDist))
            }
          } else {
            // Wall present — dark portico interior
            opacity[y*w+x] = Math.max(opacity[y*w+x], 0.58)
          ...
          if (dissolveVal < threshold) {
            ...
            // Left side wall catches golden light, right side in shadow
            const sideBr = x < porticoL ? 0.40 : 0.22
  ...
  for (let y = Math.floor(pedimentPeakY); y < Math.floor(corniceY); y++) {
    ...
    for (let x = Math.floor(porticoCX - hw); x <= Math.ceil(porticoCX + hw); x++) {
      if (x >= 0 && x < w && y >= 0 && y < h) {
        const dn = dissolveN(x, y) * 0.5 + 0.5
        // Pediment outline persists strongly
        const edgeDist = Math.abs(Math.abs(x - porticoCX) / hw - 0.95) // distance from edge
        const isEdge = edgeDist < 0.08 || t > 0.92 || t < 0.08
        const pedOp = isEdge ? 0.72 : (dn < 0.35 ? 0.28 : 0.55)
        opacity[y*w+x] = Math.max(opacity[y*w+x], pedOp)
        brightness[y*w+x] = Math.max(brightness[y*w+x], 0.36)
      }
    }
  }
    ...
    for (let dy = -haloR; dy <= haloR; dy++) {
      for (let dx = -haloR; dx <= haloR; dx++) {
        ...
        if (px >= 0 && px < w && py >= 0 && py < h) {
          ...
          if (d < 1) glow[py*w+px] = Math.max(glow[py*w+px], (1-d*d) * 0.45)
        }
      }
    }
  }
  ...
  for (let y = Math.floor(doorTop); y < Math.floor(columnBaseY); y++) {
    for (let x = Math.floor(doorCX - doorW); x <= Math.ceil(doorCX + doorW); x++) {
      if (x >= 0 && x < w && y >= 0 && y < h) {
        glow[y*w+x] = Math.max(glow[y*w+x], 0.55)
      }
    }
  }
  // Fanlight
  ...
  for (let dy = -fanlightR; dy <= 0; dy++) {
    for (let dx = -fanlightR; dx <= fanlightR; dx++) {
      ...
      if (d < fanlightR) {
        ...
        if (px >= 0 && px < w && py >= 0 && py < h) glow[py*w+px] = Math.max(glow[py*w+px], 0.40)
      }
    }
  }
  const doorHaloR = doorW * 4.5
  for (let dy = -doorHaloR; dy <= doorHaloR; dy++) {
    for (let dx = -doorHaloR; dx <= doorHaloR; dx++) {
      ...
      if (px >= 0 && px < w && py >= 0 && py < h) {
        ...
        if (d < 1) glow[py*w+px] = Math.max(glow[py*w+px], (1-d*d) * 0.30)
      }
    }
  }
  ...
  for (let si = 0; si < numSteps; si++) {
    ...
    for (let y = Math.floor(stepT); y < Math.floor(stepB); y++) {
      for (let x = Math.floor(porticoL - indent); x <= Math.ceil(porticoR + indent); x++) {
        if (x >= 0 && x < w && y >= 0 && y < h) {
          opacity[y*w+x] = Math.max(opacity[y*w+x], 0.48)
          brightness[y*w+x] = Math.max(brightness[y*w+x], si % 2 === 0 ? 0.42 : 0.32)
        }
      }
    }
  }

  // ===== LIVE OAK — MASSIVELY WIDE horizontal spread =====
  // New Orleans live oaks are wider than they are tall
  const treeCX = w * 0.15
  ...
  // Major limbs — emphasize horizontal reach
  const limbs = [
    // Far left — massive horizontal reach
    { sx: treeCX - 10, sy: trunkTopY, ex: treeCX - w*0.28, ey: h*0.40, t: 22 },
    { sx: treeCX - 5, sy: trunkTopY - 8, ex: treeCX - w*0.22, ey: h*0.30, t: 18 },
    // Upward
    { sx: treeCX, sy: trunkTopY - 12, ex: treeCX - w*0.04, ey: h*0.20, t: 14 },
    { sx: treeCX + 3, sy: trunkTopY - 10, ex: treeCX + w*0.02, ey: h*0.18, t: 12 },
    // Right — reaches toward building
    { sx: treeCX + 10, sy: trunkTopY, ex: treeCX + w*0.20, ey: h*0.36, t: 20 },
    { sx: treeCX + 8, sy: trunkTopY - 6, ex: treeCX + w*0.16, ey: h*0.28, t: 16 },
  ...
  // Canopy — VERY wide and flat, live oak signature shape
  const canopyCX = treeCX + w * 0.01
  const canopyCY = h * 0.30
  const canopyRX = w * 0.34   // extremely wide
  const canopyRY = h * 0.12   // quite flat — width >> height
  const canopyEdgeN = makeNoise(seed + 600, 30)
  const canopyGapN = makeNoise(seed + 700, 20)
  const canopyGapN2 = makeNoise(seed + 710, 35)

  for (let y = Math.max(0, Math.floor(canopyCY - canopyRY * 1.5)); y <= Math.min(h-1, Math.ceil(canopyCY + canopyRY * 1.8)); y++) {
    for (let x = Math.max(0, Math.floor(canopyCX - canopyRX * 1.15)); x <= Math.min(w-1, Math.ceil(canopyCX + canopyRX * 1.15)); x++) {
      const dx2 = (x - canopyCX) / canopyRX, dy2 = (y - canopyCY) / canopyRY
      let d = dx2*dx2 + dy2*dy2 + canopyEdgeN(x, y) * 0.32
      // Heavy bottom droop — characteristic of live oaks
      if (dy2 > 0.15) {
        const droop = (dy2 - 0.15) * 0.8
        d -= droop * (0.55 + canopyEdgeN(x*2, y) * 0.35)
      }
      if (d < 1.0) {
        const edgeFade = d > 0.55 ? (1 - d) / 0.45 : 1.0
        const gap1 = canopyGapN(x, y) * 0.5 + 0.5
        const gap2 = canopyGapN2(x, y) * 0.5 + 0.5
        const gapThresh = 0.48 + d * 0.30
        if (gap1 > gapThresh && gap2 > 0.40) {
          opacity[y*w+x] = Math.max(opacity[y*w+x], edgeFade * 0.05)
        } else {
  ...
  // Spanish moss — long draping strands
  ...
  for (let dy = -lgR; dy <= lgR; dy++) {
    for (let dx = -lgR; dx <= lgR; dx++) {
      ...
      if (px >= 0 && px < w && py >= 0 && py < h) {
        ...
        if (d2 < 1) glow[py*w+px] = Math.max(glow[py*w+px], (1-d2*d2) * 0.45)
      }
    }
  }
...
function buildIron(w: number, h: number, seed: number) {
  ...
  // Simple iron picket fence — lower in frame, subordinate to building
...
async function main() {
  ...
  console.log(`=== NOSTALGIA v24 variant ${variant} (seed: ${seed}) ===`)
  const canvas = createCanvas(W, H)
  const ctx = canvas.getContext("2d")

  // Sky — warmer golden hour
  ...
  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W; x++) {
      ...
      // Warmer sky gradient — more amber/gold, less orange
  ...
  // LESS blur — building should be more recognizable
  const opB = gaussianBlur(opacity, W, H, 4)
  const brB = gaussianBlur(brightness, W, H, 5)
  const glB = gaussianBlur(glowMap, W, H, 25)
  ...
  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W; x++) {
      ...
      const br = brB[i]
      if (op > 0.01) {
        const dR = 38 + tex*12, dG = 28 + tex*8, dB = 24 + tex*6
        const aR = 200 + tex*20, aG = 155 + tex*15, aB = 88 + tex*10
      ...
      const gl = glB[i]
      if (gl > 0.01) {
        const ga = Math.min(0.75, gl * 0.75)
        r = r + (255-r)*ga
        g = g + (200-g)*ga
        b = b + (95-b)*ga
      }
  ...
  const fn = `output/nostalgia-v24-${variant}.png`
