/**
 * CONVERSATION PIECE v3 — refinements on v2's time-split
 *
 * v2 established: inside the window it is still noon (GR7, four flat colours,
 * sun up); outside, the same scene at dusk, sun down, afterglow where it was.
 *
 * v3 adds:
 *  - Some dusk flowers have CLOSED for the night (drooped dark buds). In the
 *    memory, every flower is still open.
 *  - Optional first stars in the upper violet (variant flag — judge honestly).
 *  - Two window sizes (80×48, 70×42 — both 4:3 on screen) and more seeds,
 *    because chance draws and I curate.
 *  - Stronger sky grain against banding.
 */
...
// window sizes set per-variant; aw/ah = 5/3 keeps the region 4:3 on screen
...
// ---------------------------------------------------------------------------
function renderDusk(scene: Scene, seed: number, stars: boolean): Float32Array {
	...
	// flowers at dusk — some have closed for the night; the open ones keep
	// their true colours, dimmed, catching the last light
	const rnd = makePRNG(seed * 13 + 5)
	for (const f of scene.flowers) {
		const closed = rnd() < 0.4
		if (closed) {
			// drooped stem with a small dark bud — colour drained to near-silhouette
			const fx = f.x * W
			const fy = f.y * H
			const depth = (f.y - scene.groundY) / (1 - scene.groundY)
			const size = f.size * CW * (0.5 + depth * 0.9)
			const stemH = size * 2.6
			const budCol: RGB = mix(atariRGB(f.trueHue, 4) as RGB, [30, 30, 24], 0.65)
			const stemCol: RGB = [34, 36, 27]
			for (let t = 0; t < stemH; t++) {
				const droop = (t / stemH) ** 2 * size * 0.9 // curls over near the top
...
function placeWindow(scene: Scene, aw: number, ah: number): { ax0: number; ay0: number; aw: number; ah: number } {
	const sunAx = Math.round(scene.sun.x * AW)
	const sunAy = Math.round(scene.sun.y * AH)
	const groundRow = Math.round(scene.groundY * AH)
	let ay0 = groundRow + 10 - ah // 10 rows of ground inside
	if (sunAy - 5 < ay0) ay0 = Math.max(2, sunAy - 5) // but never lose the sun
	ay0 = Math.min(ay0, AH - ah - 2)
	let ax0 = sunAx - Math.round(aw * 0.62) // sun right-of-centre in the window
	ax0 = Math.max(4, Math.min(AW - aw - 4, ax0))
	return { ax0, ay0, aw, ah }
}

mkdirSync("output", { recursive: true })
const variants: Array<{ seed: number; aw: number; ah: number; stars: boolean }> = [
	{ seed: 101, aw: 80, ah: 48, stars: false },
	{ seed: 101, aw: 70, ah: 42, stars: true },
	{ seed: 202, aw: 70, ah: 42, stars: false },
	{ seed: 303, aw: 80, ah: 48, stars: false },
	{ seed: 505, aw: 70, ah: 42, stars: true },
	{ seed: 606, aw: 80, ah: 48, stars: false },
	{ seed: 707, aw: 70, ah: 42, stars: false },
	{ seed: 808, aw: 80, ah: 48, stars: true },
	{ seed: 909, aw: 70, ah: 42, stars: false },
]
for (const v of variants) {
	const scene = drawScene(v.seed)
	const atari = renderAtari(scene)
	const dusk = renderDusk(scene, v.seed, v.stars)
	const label = `s${v.seed}-${v.aw}${v.stars ? "-stars" : ""}`
	composite(scene, atari, dusk, placeWindow(scene, v.aw, v.ah), `output/conversation-piece-v3-${label}.png`)
}
