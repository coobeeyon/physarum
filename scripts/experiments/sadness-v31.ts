/**
 * SADNESS v31 — REPEATED FAILURE, revised against the first blind read.
 *
 * v30's blind read (art/blind-reads/2026-08-24.md): the emotion landed
 * ("hunched bodies... one person's posture repeated as they diminish") but
 * the image failed on presence — "low contrast, vast empty area, indistinct
 * forms... an unfinished gesture."
 *
 * Changes, each answering a specific line of that critique:
 * - Deeper, darker ink (contrast): burnt sienna core -> dark grey death,
 *   both far from the cream ground.
 * - Crisp edges while a stroke is strong; fraying reserved for its failure
 *   (precision — "frayed = faded" worked emotionally, uniform fuzz didn't).
 * - The three attempts spread rightward across the canvas so the figures
 *   walk INTO the emptiness — the empty space becomes where the sequence is
 *   headed, not leftover canvas.
 * - Fewer, smaller fallen fragments (droplets read as noise).
 */
...
async function main() {
  ...
  console.log(`=== SADNESS v31 variant ${variant} (seed: ${seed}) ===`)
  ...
  const marks: MarkConfig[] = [
    // Mark 1: Strong first attempt
    {
      startX: baseStartX,
      startY: H + 30,
      velocity: 11 + rand() * 2,        // strong upward — its apex measures the space above
      vxBias: 1.5 + rand() * 1.2,
  ...
  const numFragments = 8 + Math.floor(rand() * 5)
  ...
  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W; x++) {
      ...
      // Deep burnt sienna → dark ash grey. Both ends sit far from the cream
      // ground so the figures are present even as they fade (the v30 read
      // called the forms "insubstantial" — fading should live in the edges
      // and gaps, not in an overall lack of pigment).
      const warmR = 118 + warmth * 32
      const warmG = 48 + warmth * 16
      const warmB = 16 + warmth * 8
      const coldR = 98
      const coldG = 92
      const coldB = 88
  ...
  const filename = `output/sadness-v31-${variant}.png`
